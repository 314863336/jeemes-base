...
import com.huitai.license.utils.*;
 ...
 * version: 1.0 <br>
 */
@Api(value = "LicenseServerController接口")
...
public class LicenseController {
    ...
    @Autowired
    private LicenseServerUtil licenseServerUtil;
     ...
     * author: XJM <br>
     */
    @ApiOperation(value = "generator",notes="")
    @RequestMapping(value="/generator",method = {RequestMethod.POST})
    public Result generator(@RequestBody LicenseChecker licenseChecker){
        LicenseCreator licenseCreator = new LicenseCreator();

        licenseCreator.setSubject(subject);
        if(LicenseConstant.USER_DEVELOP.equals(licenseChecker.getUserType())){
            licenseChecker.setStartDate(new Date());
        }
        licenseCreator.setLicenseChecker(licenseChecker);

        licenseServerUtil.generateLicense(licenseCreator);
        return Result.ok();
    }
